...
def load_credentials(config_file="smb_config.ini"):
    ...
    domain = config.get("auth", "domain", fallback="WORKGROUP")
    return username, password, domain

def smb_enum(ip):
    smb_user, smb_pass, smb_domain = load_credentials()
    try:
        conn = SMBConnection(
            smb_user,
            smb_pass,
        ...
        if conn.connect(ip, 139, timeout=10):
            auth_type = "Authenticated" if smb_user else "Anonymous"
            print(f"[+] {auth_type} login success: {ip}")
            ...
            print(f"[-] Failed to connect to SMB on {ip}")
        conn.close()
