...
# Check if dependencies and platform support are proper
...
try:
    ...
    if platform.system().lower() == "windows":
        ...
        print("[!] Installation Error. Please install pysmb manually!!")
    from smb.SMBConnection import SMBConnection
...
# Port Scan for default SMB ports
...
# ✅ MODIFIED FUNCTION: SMB Enumeration with Windows Session Auth
def smb_enum(ip):
    import getpass
    import os
    ...
    password = ""  # Empty password triggers NTLM passthrough (session-based)

    try:
        conn = SMBConnection(
            username, password, "autoenum", "target",
            domain=domain, use_ntlm_v2=True, is_direct_tcp=True
        )
        if conn.connect(ip, 445, timeout=10):
            print(f"[+] Authenticated login success: {ip}")
...
# Recursive Download files to local directory
