...
def generate_report(data, report_dir="reports"):
    os.makedirs(report_dir, exist_ok=True)

    print("[SMBRUNNERv2] Report format? (txt/json/html, leave blank for ALL): ", end="", flush=True)
    try:
    ...
    if not report_format:
        # No input → generate all formats
        formats = ["txt", "json", "html"]
        print("[+] No format selected. Generating reports in all formats...")
    else:
        # Validate
        if report_format not in ["txt", "json", "html"]:
            print("[!] Invalid format. Defaulting to all formats...")
            formats = ["txt", "json", "html"]
        else:
    ...
    generated = []
    for fmt in formats:
        filename = os.path.join(report_dir, f"report_{datetime.now().strftime('%Y-%m-%d_%H%M')}.{fmt}")
        try:
            if fmt == "txt":
                with open(filename, "w") as f:
                    f.write("===== SMBRUNNERv2 Report =====\n")
                    f.write(f"Live Hosts: {len(data.get('live_hosts', []))}\n")
                    f.write(f"Open Ports: {len(data.get('open_ports', []))}\n")
                    f.write(f"Secrets: {len(data.get('secrets', []))}\n")
            elif fmt == "json":
                with open(filename, "w") as f:
                    json.dump(data, f, indent=4)
            elif fmt == "html":
                with open(filename, "w") as f:
                    f.write("<html><head><title>SMBRUNNERv2 Report</title></head><body>")
                    f.write("<h1>SMBRUNNERv2 Report</h1>")
                    f.write(f"<p>Live Hosts: {len(data.get('live_hosts', []))}</p>")
                    f.write(f"<p>Open Ports: {len(data.get('open_ports', []))}</p>")
                    f.write(f"<p>Secrets Found: {len(data.get('secrets', []))}</p>")
                    f.write("</body></html>")
